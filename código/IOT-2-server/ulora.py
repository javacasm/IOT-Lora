...
from machine import SoftSPI,SPI
...
class LoRa(object):
    def __init__(self, spi_sck, spi_mosi, spi_miso, interrupt, this_address, cs_pin, reset_pin=None, freq=868.0, tx_power=14,
        ...
        print(f'Receive_all{self._receive_all}')
        self._acks = acks

        self._this_address = this_address
        print(f'Address: {self._this_address}')
...
#        gpio_interrupt = Pin(self._interrupt, Pin.IN, Pin.PULL_DOWN)
        ...
        # reset the board
        if self._reset_pin:
            print(f'reset RFM95 {self._reset_pin}')
        ...
        print(f'SPI sck:{self._spi_sck} MISO:{self._spi_miso} MOSI:{self._spi_mosi} CS:{self._cs_pin}') 
 
        # baud rate to 5MHz
        self.spi =  SoftSPI(baudrate = 10000000, 
            polarity = 0, phase = 0, bits = 8, firstbit = SPI.MSB,
            sck = Pin(self._spi_sck, Pin.OUT, Pin.PULL_DOWN),
            mosi = Pin(self._spi_mosi, Pin.OUT, Pin.PULL_UP),
            miso = Pin(self._spi_miso, Pin.IN, Pin.PULL_UP))
        # cs gpio pin
    ...
    def _handle_interrupt(self, channel):
        ...
        print(f'Interrupt mode:{self._mode}')
        ...
        if self._mode == MODE_RXCONTINUOUS and (irq_flags & RX_DONE):
            ...
            if packet_len >= 4:
                ...
                if (self._this_address != header_to) and (header_to != BROADCAST_ADDRESS) and (self._receive_all is False): 
                    print(f'message not for me: {header_to} recv_all:{self._receive_all}')
                    return
